...
Users enter one visible **display name** on the homepage. Existing rooms are joined from room links; the homepage does not ask for lobby names. Identity is a hidden random auth token stored in browser localStorage alongside that display name. Server room state is in memory and may reset on process restart; the browser's auth token/display name remains available locally. If duplicate display names join the same room, stable room-scoped numbers are appended automatically. Rooms support up to 20 active seated players plus 200 inactive spectators/observers.
